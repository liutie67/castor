...
vDataFile::vDataFile() 
{
  ...
  // Variable for additional data
  m_loadedAdditional = false;
  m2p_additionalData = NULL;
  m_nbAdditionalData = -1;
}
...
int vDataFile::InitAdditionalData(const vector<string>& a_pathToImage, vDataFile** ap_DataFile)
{
  // Allocate and read only if a file is provided
  if (!a_pathToImage.empty())
  {
    // Allocate memory (if not already done)
    if (!m_loadedAdditional)
    {
      m2p_additionalData = (FLTNB**)malloc(GetNbAdditionalData()*sizeof(FLTNB*));
      for (int nb=0; nb<GetNbAdditionalData(); nb++)
      {
        // Verbose
        if (m_verbose>=3) Cout("vDataFile::InitAdditionalData() -> From file '" << a_pathToImage.at(nb) << "'"<< endl);
        // Open file
        ifstream image_file(a_pathToImage.at(nb).c_str(), ios::in|ios::binary);
        if (!image_file.is_open())
        {
          Cerr("***** vDataFile::InitAdditionalData() -> Input file '" << a_pathToImage.at(nb) << "' is missing or corrupted !" << endl);
          // Read failure implies that the additional data will never be used, so free all the allocated memory
          DeallocateAdditionalData();
          return 1;
        }
        m2p_additionalData[nb] = (FLTNB*)malloc(GetSinogramSize()*sizeof(FLTNB));
        // Interfile data reading (interpolation not allowed)

        if (IntfReadSinogram(a_pathToImage.at(nb), m2p_additionalData[nb], ap_DataFile, m_verbose, false))
        {
          Cerr("***** vDataFile::InitAdditionalData() -> Error reading interfile image '" << a_pathToImage.at(nb) << "' !" << endl);
          // Read failure implies that the additional data will never be used, so free all the allocated memory
          DeallocateAdditionalData();
          return 1;
        }
...
void vDataFile::DeallocateAdditionalData()
{
  // Check the first pointer
  if (m2p_additionalData)
  {
    // Verbose
    if (m_verbose>=3) Cout("vDataFile::DeallocateAdditionalData() -> Free memory" << endl);
    for (int nb=0; nb<GetNbAdditionalData(); nb++) if (m2p_additionalData[nb]) free(m2p_additionalData[nb]);
    free(m2p_additionalData);
  }
...
vEvent* vDataFile::GetEvent(int64_t a_eventIndex, int a_th)
{
  ...
  p_event->SetEventIndex(a_eventIndex);
  // Return the event
  return p_event;
}
