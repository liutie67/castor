...
/*!
  \file
  \ingroup  optimizer
  \brief    Implementation of class iOptimizerADMMLim_dirty1
*/

#include "iOptimizerADMMLim_dirty1.hh"
...
iOptimizerADMMLim_dirty1::iOptimizerADMMLim_dirty1() : vOptimizer()
{
  // ---------------------------
  ...
  // Initial value at 1
  m_initialValue = 1.;
  // Only one backward image for ADMMLim_dirty1
  m_nbBackwardImages = 1;
  // ADMMLim_dirty1 accepts penalties
  m_requiredPenaltyDerivativesOrder = 1;
  // ADMMLim_dirty1 is compatible with listmode and histogram data (not yet)
  m_listmodeCompatibility = false;
  m_histogramCompatibility = true;
  // ADMMLim_dirty1 is specific to emission data
  m_emissionCompatibility = true;
  m_transmissionCompatibility = false;

  // ADMMLim_dirty1 needs to use the post process loop to compute v and u (one iteration needed for both)
...
iOptimizerADMMLim_dirty1::~iOptimizerADMMLim_dirty1()
{
...
void iOptimizerADMMLim_dirty1::ShowHelpSpecific()
{
...
int iOptimizerADMMLim_dirty1::ReadConfigurationFile(const string& a_configurationFile)
{
  string key_word = "";
  ...
  {
    Cerr("***** iOptimizerADMMLim_dirty1::ReadConfigurationFile() -> Failed to get the '" << key_word << "' keyword !" << endl);
    return 1;
  }
  // Normal end
  return 0;
...
int iOptimizerADMMLim_dirty1::ReadOptionsList(const string& a_optionsList)
{
  // There are 1 floating point variable as option
  const int nb_options = 1;
  FLTNB options[nb_options];
  // Read them
  if (ReadStringOption(a_optionsList, options, nb_options, ",", "ADMMLim_dirty1 configuration"))
  {
    Cerr("***** iOptimizerADMMLim_dirty1::ReadOptionsList() -> Failed to correctly read the list of options !" << endl);
    return 1;
  }
  // Affect option
  m_alpha = options[0];
...
int iOptimizerADMMLim_dirty1::CheckSpecificParameters()
{
  // Check that penalty parameter alpha is positive
  if (m_alpha<=0.)
  {
    Cerr("***** iOptimizerADMMLim_dirty1->CheckSpecificParameters() -> Provided alpha (" << m_alpha << ") must be strictly positive !" << endl);
    return 1;
  }
  // Cannot deal with list-mode or transmission data
  if (m_dataSpec==SPEC_TRANSMISSION || m_dataMode==MODE_LIST)
  {
    Cerr("***** iOptimizerADMMLim_dirty1->CheckSpecificParameters() -> Cannot reconstruct list-mode or transmission data !" << endl);
    return 1;
  }
  ...
  {
    Cerr("***** iOptimizerADMMLim_dirty1::CheckSpecificParameters() -> ADMMLim_dirty1 requires 2 additional data for u^k and v^k ! (we will zero them, assuming you want to initialize v^0...)"<< endl);
  }
  else if (mp_DataFile->GetNbAdditionalData()!=2)
  {
    Cerr("***** iOptimizerADMMLim_dirty1::CheckSpecificParameters() -> ADMMLim_dirty1 requires 2 additional data for u^k and v^k !"<< endl);
    return 1;
  }
  // Normal end
  return 0;
...
int iOptimizerADMMLim_dirty1::InitializeSpecific()
{
  ...
  {
    Cout("iOptimizerADMMLim_dirty1::InitializeSpecific() -> Use the ADMMLim_dirty1 optimizer" << endl);
...
int iOptimizerADMMLim_dirty1::DataStep4Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
...
int iOptimizerADMMLim_dirty1::SensitivitySpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_weight,
...
int iOptimizerADMMLim_dirty1::DataStep5ComputeCorrections( oProjectionLine* ap_Line, vEvent* ap_Event,
                                             int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                             int a_th )
{
  // Loop on TOF bins
...
int iOptimizerADMMLim_dirty1::DataStep6Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
                                   int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                   int a_th )
{
...
int iOptimizerADMMLim_dirty1::DataSpaceSpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_backwardValues,
                                                 FLTNB a_multiplicativeCorrections, FLTNB a_additiveCorrections, FLTNB a_blankValue,
                                                 FLTNB a_quantificationFactor, oProjectionLine* ap_Line )
{
...
int iOptimizerADMMLim_dirty1::PreImageUpdateSpecificStep()
{
  // ==========================================================================================
  ...
  #endif
  // Verbose
  if (m_verbose>=1) Cout("iOptimizerADMMLim_dirty1::PreImageUpdateSpecificStep() -> Compute penalty term" << endl);
  // ==========================================================================================
  // Global precomputation step if needed by the penalty
  if (mp_Penalty->GlobalPreProcessingStep())
  {
    Cerr("***** iOptimizerADMMLim_dirty1::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        { 
          ...
          {
            Cerr("***** iOptimizerADMMLim_dirty1::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
        ...
        if (problem)
        {
          Cerr("***** iOptimizerADMMLim_dirty1::PreImageUpdateSpecificStep() -> A problem occurred inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
...
int iOptimizerADMMLim_dirty1::ImageSpaceSpecificOperations( FLTNB a_currentImageValue, FLTNB* ap_newImageValue,
