...
/*!
  \file
  \ingroup  optimizer
  \brief    Implementation of class iOptimizerADMMLim_adaptiveRhoTau
*/

#include "iOptimizerADMMLim_adaptiveRhoTau.hh"
...
iOptimizerADMMLim_adaptiveRhoTau::iOptimizerADMMLim_adaptiveRhoTau() : vOptimizer()
{
  // ---------------------------
...
iOptimizerADMMLim_adaptiveRhoTau::~iOptimizerADMMLim_adaptiveRhoTau()
{
...
void iOptimizerADMMLim_adaptiveRhoTau::ShowHelpSpecific()
{
...
int iOptimizerADMMLim_adaptiveRhoTau::ReadConfigurationFile(const string& a_configurationFile)
{
  string key_word = "";
...
int iOptimizerADMMLim_adaptiveRhoTau::ReadOptionsList(const string& a_optionsList)
{
...
int iOptimizerADMMLim_adaptiveRhoTau::CheckSpecificParameters()
{
...
int iOptimizerADMMLim_adaptiveRhoTau::InitializeSpecific()
{
...
int iOptimizerADMMLim_adaptiveRhoTau::DataStep4Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
...
int iOptimizerADMMLim_adaptiveRhoTau::SensitivitySpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_weight,
...
int iOptimizerADMMLim_adaptiveRhoTau::DataStep5ComputeCorrections( oProjectionLine* ap_Line, vEvent* ap_Event,
                                             int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                             int a_th )
{
  // Loop on TOF bins
...
int iOptimizerADMMLim_adaptiveRhoTau::DataStep6Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
                                   int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                   int a_th )
{
...
int iOptimizerADMMLim_adaptiveRhoTau::DataSpaceSpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_backwardValues,
                                                 FLTNB a_multiplicativeCorrections, FLTNB a_additiveCorrections, FLTNB a_blankValue,
                                                 FLTNB a_quantificationFactor, oProjectionLine* ap_Line )
{
...
int iOptimizerADMMLim_adaptiveRhoTau::PreImageUpdateSpecificStep()
{
  if (m_isInPostProcessLoop)  // only in post process loop
  {
  ...
  #endif
  // Verbose
  if (m_verbose>=1) Cout("iOptimizerADMMLim_adaptiveRhoTau::PreImageUpdateSpecificStep() -> Compute penalty term" << endl);
  // ==========================================================================================
  // Global precomputation step if needed by the penalty
  if (mp_Penalty->GlobalPreProcessingStep())
  {
    Cerr("***** iOptimizerADMMLim_adaptiveRhoTau::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        { 
          ...
          {
            Cerr("***** iOptimizerADMMLim_adaptiveRhoTau::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
        ...
        if (problem)
        {
          Cerr("***** iOptimizerADMMLim_adaptiveRhoTau::PreImageUpdateSpecificStep() -> A problem occurred inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
...
int iOptimizerADMMLim_adaptiveRhoTau::ImageSpaceSpecificOperations( FLTNB a_currentImageValue, FLTNB* ap_newImageValue,
