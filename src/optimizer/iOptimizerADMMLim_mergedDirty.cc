...
/*!
  \file
  \ingroup  optimizer
  \brief    Implementation of class iOptimizerADMMLim_mergedDirty
*/

#include "iOptimizerADMMLim_mergedDirty.hh"
...
iOptimizerADMMLim_mergedDirty::iOptimizerADMMLim_mergedDirty() : vOptimizer()
{
  // ---------------------------
...
iOptimizerADMMLim_mergedDirty::~iOptimizerADMMLim_mergedDirty()
{
...
void iOptimizerADMMLim_mergedDirty::ShowHelpSpecific()
{
...
int iOptimizerADMMLim_mergedDirty::ReadConfigurationFile(const string& a_configurationFile)
{
  string key_word = "";
  ...
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty::ReadConfigurationFile() -> Failed to get the '" << key_word << "' keyword !" << endl);
    return 1;
  }
  // Normal end
  return 0;
...
int iOptimizerADMMLim_mergedDirty::ReadOptionsList(const string& a_optionsList)
{
  ...
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty::ReadOptionsList() -> Failed to correctly read the list of options !" << endl);
    return 1;
  }
  // Affect option
  m_alpha = options[0];
...
int iOptimizerADMMLim_mergedDirty::CheckSpecificParameters()
{
  // Check that penalty parameter alpha is positive
  if (m_alpha<=0.)
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty->CheckSpecificParameters() -> Provided alpha (" << m_alpha << ") must be strictly positive !" << endl);
    return 1;
  }
  // Cannot deal with list-mode or transmission data
  if (m_dataSpec==SPEC_TRANSMISSION || m_dataMode==MODE_LIST)
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty->CheckSpecificParameters() -> Cannot reconstruct list-mode or transmission data !" << endl);
    return 1;
  }
  ...
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty::CheckSpecificParameters() -> ADMMLim requires 2 additional data for u^k and v^k ! (we will zero them, assuming you want to initialize v^0...)"<< endl);
  }
  else if (mp_DataFile->GetNbAdditionalData()!=2)
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty::CheckSpecificParameters() -> ADMMLim requires 2 additional data for u^k and v^k !"<< endl);
    return 1;
  }
  // Normal end
  return 0;
...
int iOptimizerADMMLim_mergedDirty::InitializeSpecific()
{
  ...
  {
    Cout("iOptimizerADMMLim_mergedDirty::InitializeSpecific() -> Use the ADMMLim optimizer" << endl);
...
int iOptimizerADMMLim_mergedDirty::DataStep4Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
...
int iOptimizerADMMLim_mergedDirty::SensitivitySpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_weight,
...
int iOptimizerADMMLim_mergedDirty::DataStep5ComputeCorrections( oProjectionLine* ap_Line, vEvent* ap_Event,
                                             int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                             int a_th )
{
  // Loop on TOF bins
  for (int b=0; b<ap_Line->GetNbTOFBins(); b++)
  {
    ...
    *ap_backwardValues = m_AxProduct[a_th] - (HPFLTNB)m_vk[a_th] + (HPFLTNB)m_uk[a_th];
    /*
    if (ap_Line->GetEventIndex()%1000==0){cout << "---*m_AxProduct[a_th]------*m_AxProduct[a_th]---" << m_AxProduct[a_th] << endl;}
    if (ap_Line->GetEventIndex()%1000==0){cout << "-----------------*m_vk[a_th]------*m_vk[a_th]---" << m_vk[a_th] << endl;}
    if (ap_Line->GetEventIndex()%1000==0){cout << "-----------------*m_uk[a_th]------*m_uk[a_th]---" << m_uk[a_th] << endl;}
    if (ap_Line->GetEventIndex()%1000==0){cout << "---*ap_backwardValues------*ap_backwardValues---" << *ap_backwardValues << endl;}
    */
...
int iOptimizerADMMLim_mergedDirty::DataStep6Optional( oProjectionLine* ap_Line, vEvent* ap_Event,
                                   int a_bed, int a_timeFrame, int a_respGate, int a_cardGate,
                                   int a_th )
{
...
int iOptimizerADMMLim_mergedDirty::DataSpaceSpecificOperations( FLTNB a_data, FLTNB a_forwardModel, FLTNB* ap_backwardValues,
                                                 FLTNB a_multiplicativeCorrections, FLTNB a_additiveCorrections, FLTNB a_blankValue,
                                                 FLTNB a_quantificationFactor, oProjectionLine* ap_Line )
{
...
int iOptimizerADMMLim_mergedDirty::PreImageUpdateSpecificStep()
{
  // ==========================================================================================
  ...
  #endif
  // Verbose
  if (m_verbose>=1) Cout("iOptimizerADMMLim_mergedDirty::PreImageUpdateSpecificStep() -> Compute penalty term" << endl);
  // ==========================================================================================
  // Global precomputation step if needed by the penalty
  if (mp_Penalty->GlobalPreProcessingStep())
  {
    Cerr("***** iOptimizerADMMLim_mergedDirty::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty pre-processing step !" << endl);
    return 1;
  }
  // ==========================================================================================
  // Loop over time basis functions
    ...
    for (int rbf=0; rbf<mp_ImageDimensionsAndQuantification->GetNbRespBasisFunctions(); rbf++)
    {
      ...
      for (int cbf=0; cbf<mp_ImageDimensionsAndQuantification->GetNbCardBasisFunctions(); cbf++)
      {
        ...
        { 
          ...
          {
            Cerr("***** iOptimizerADMMLim_mergedDirty::PreImageUpdateSpecificStep() -> A problem occurred while computing the penalty local pre-processing step for voxel " << v << " !" << endl);
            problem = true;
          }
        ...
        if (problem)
        {
          Cerr("***** iOptimizerADMMLim_mergedDirty::PreImageUpdateSpecificStep() -> A problem occurred inside the multi-threaded loop, stop now !" << endl);
          return 1;
        }
      }
    }
...
int iOptimizerADMMLim_mergedDirty::ImageSpaceSpecificOperations( FLTNB a_currentImageValue, FLTNB* ap_newImageValue,
