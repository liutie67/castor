...
int vAlgorithm::StepBeforeIterationLoop()
{
  ...
  // Instantiate all the required images and sinograms from the oImageSpace and the vDataFile
  ...
  if (m2p_DataFile[0]->InitAdditionalData(m_pathToAdditionalData, m2p_DataFile))
  {
    Cerr("***** vAlgorithm::StepBeforeIterationLoop()-> Error during additional data initialization !" << endl);
    return 1;
  }
...
int vAlgorithm::StepAfterIterationLoop()
{
  ...
  m2p_DataFile[0]->DeallocateAdditionalData();
