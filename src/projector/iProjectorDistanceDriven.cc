...
int iProjectorDistanceDriven::ReadOptionsList(const string& a_optionsList)
{
  ...
  {
    Cerr("***** iProjectorDistanceDriven::ReadConfigurationFile() -> Failed to correctly read the list of options !" << endl);
    return 1;
  }
  ...
  if (option[0] < 2.)
  {
    Cerr("***** iProjectorDistanceDriven::ReadOptionsList() -> Tolerance factor parameter must be  >2 (tolerance factor < 10^-2)" << endl);
...
int iProjectorDistanceDriven::CheckSpecificParameters()
{
  if ( m_tolerance_fctr >  0.01 
    || m_tolerance_fctr <= 0 )
  {
    Cerr("***** iProjectorDistanceDriven::CheckSpecificParameters() -> Tolerance factor must be defined in the specific interval : ]0 ; 1.e-2]" << endl);
...
int iProjectorDistanceDriven::ProjectWithoutTOF(int a_direction, oProjectionLine* ap_ProjectionLine )
{
  ...
  if( ( angle >= 0.0 && angle <= 45.0 ) || ( angle >= 135.0 && angle <= 180.0 ) )
  {
    ...
    for( int16_t i = i_min; i < i_max; ++i )
    {
      ...
      if (test_index && mp_ImageDimensionsAndQuantification->IsVoxelMasked(test_index1) && mp_ImageDimensionsAndQuantification->IsVoxelMasked(test_index2)) continue;
