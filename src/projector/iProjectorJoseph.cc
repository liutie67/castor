...
int iProjectorJoseph::ProjectWithoutTOF(int a_direction, oProjectionLine* ap_ProjectionLine )
{
  ...
  if( ::fabs( r[ 0 ] ) > ::fabs( r[ 1 ] ) )
  {
    ...
    for( int i = iMin; i < iMax; ++i )
    {
      ...
      // If the main voxel and all the neighbours are masked and inside the FOV, skip
      // Try to avoid as much computations as possible and be on the safe side
      // (better to include some masked voxels than to skip some relevant voxels)
      if ( limitY1 && limitZ1 && limitY2 && limitZ2 && 
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx) && 
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + m_nbVoxXY) &&
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + mp_nbVox[ 0 ] + m_nbVoxXY) &&
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + mp_nbVox[ 0 ]) ) continue;
      ...
      // Add voxels and their weights to the projection line
  ...
  else
  {
    ...
    for( int j = jMin; j < jMax; ++j )
    {
      ...
      // Try to avoid as much computations as possible and be on the safe side
      // (better to include some masked voxels than to skip some relevant voxels)
      if ( limitX1 && limitZ1 && limitX2 && limitZ2 && 
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx) && 
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + m_nbVoxXY) &&
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + 1 + m_nbVoxXY) &&
           mp_ImageDimensionsAndQuantification->IsVoxelMasked(finalIdx + 1) ) continue;
