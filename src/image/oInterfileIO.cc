...
int IntfReadSinogram(   const string& a_pathToHeaderFile, 
                         FLTNB* ap_DataMatrix,
...
{
  // Verbose
  if (a_verbose>=VERBOSE_DETAIL) Cout("oInterfileIO::IntfReadSinogram() -> Read sinogram from interfile header '" << a_pathToHeaderFile << "'" << endl);
  ...
  if (IntfReadHeader(a_pathToHeaderFile, &Img_fields, a_verbose) )
  {
    Cerr("***** oInterfileIO::IntfReadSinogram() -> An error occurred while trying to read the interfile header '" << a_pathToHeaderFile << "' !" << endl);
    return 1;
  }
  ...
  /*if (IntfCheckConsistency(&Img_fields, ap_ID, a_verbose, a_lerpFlag) )
  {
    Cerr("***** oInterfileIO::IntfReadSinogram() -> An error occurred while checking consistencies between reconstruction parameters and interfile keys from header '"
         << a_pathToHeaderFile << "' !" << endl);
    return 1;
  }
  */
  ...
  if (img_file)
  {
    ...
    uint32_t offset = Img_fields.data_offset;
    // Call the right IntfReadData() function according to the pixel type, and read data
    if (IntfGetPixelTypeAndReadData(Img_fields, &img_file, ap_DataMatrix, pimg_erp, &offset, ap_Datafile[0]->GetSinogramSize(), a_verbose))
    {
      Cerr("***** oInterfileIO::IntfReadSinogram() -> An error occurred while reading data and eventually interpolating from the IntfGetPixelTypeAndReadData() function !" << endl);
      return 1;
    }
  }
  else
  {
    Cerr("***** oInterfileIO::IntfReadSinogram() -> An error occurred while trying to open the image file '" << Img_fields.path_to_image << "' !" << endl);
    // If interpolation was required, deallocate image matrix
    if (pimg_erp) delete[] pimg_erp; 
    return 1;
  }
