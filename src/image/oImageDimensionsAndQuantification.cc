...
int oImageDimensionsAndQuantification::ProcessAndSetMask(FLTNB* ap_maskImage)
{
  ...
  if (m_hasMask)
  {
    Cerr("***** oImageDimensionsAndQuantification::ProcessAndSetMask() -> Mask already initialized !" << endl);
    return 1;
  }
  // Allocate
  mp_mask = new bool[m_nbVoxXYZ];
  // Fill the mask
  for (INTNB v=0; v<m_nbVoxXYZ;v++)
  {
...
{
  return (m_hasMask && !mp_mask[a_voxIndex]);
}
