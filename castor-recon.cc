...
//MULTIBED  cout << "  -df file.cdf         : Give an input CASTOR datafile (no default). Can use this option multiple times to specify multiple bed positions." << endl;
  ...
  cout << endl;
  cout << "  -additional-data file.hdr : Provide additional data, with sinogram size, for use in reconstruction." << endl;
  cout << "                         Multiple additional sinograms can be provided by using the option multiple times." << endl;
  cout << endl;
  cout << "  -mask file.hdr       : Provide a mask image for avoiding computations (projection, convolution, penalty) on irrelevant voxels (background)."<<endl; 
  cout << "                         The mask should contain zero values for the background and non zero values for relevant voxels." << endl;
  cout << endl;
...
int main(int argc, char** argv)
{
  ...
  // Paths to additional images coming from -multimodal option
  vector<string> path_to_multimodal_img;
  // Paths to additional data coming from -additional-data option
  vector<string> path_to_additional_data;  // Path to an image used as mask
  ...
  for (int i=1; i<argc; i++)
  {
    ...
    // Additional images (sinogram size)
    else if (option=="-additional-data")
    {
      if (i>=argc-1)
      {
        Cerr("***** Argument missing for option: " << option << endl);
      ...
      string file_name = (string)argv[i+1];
      path_to_additional_data.push_back(file_name);
      i++;
    }
    // Mask image
    else if (option=="-mask")
  ...
  */
  ...
  // ----------------------------------------------------------------------------------------
  // If mask image provided, load it using ImageSpace and store it into ImageDimensionsAndQuantification
  // ----------------------------------------------------------------------------------------
  ...
  if (!path_to_mask_img.empty())
  {
    ...
    {
      Cerr("The option for using a mask image is set, but the mask image is not provided or could not be loaded !" << endl);
      Exit(EXIT_FAILURE);
    }
    p_ImageDimensionsAndQuantification->ProcessAndSetMask(p_ImageSpace->mp_maskImage);
  ...
  for (int bed=0 ; bed<nb_beds ; bed++)
  {
    ...
    p_DataFile[bed]->SetNbAdditionalData(path_to_additional_data.size());
    
    // Get the number of events in the data from the header (to set sinogram size for ADMM)
    int64_t nb_events = 0;
    ReadDataASCIIFile(path_to_data_filename[0], "Number of events", &nb_events, 1, KEYWORD_MANDATORY);
    p_DataFile[bed]->SetSinogramSize((INTNB)nb_events);
  ...
  p_OptimizerManager->SetDataFile(p_DataFile[0]);
  ...
  p_Algorithm->SetPathToAdditionalData(path_to_additional_data);
